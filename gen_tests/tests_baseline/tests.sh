...
if [[ $benchmarks_read == 1 ]]
then
  mkdir -p res/bandwidth_rw
  sudo echo 0 > /sys/kernel/debug/tracing/trace

  bandwidth -c 0 -t 1000 >> bw_read.txt & 

  sleep 5
    
  sudo cat /sys/kernel/debug/tracing/trace > trace_bandwidth_read.txt
  ...
  bandwidth -a write -c 0 -t 1000 >> bw_write.txt & 

  sleep 5
    
  sudo cat /sys/kernel/debug/tracing/trace > trace_bandwidth_write.txt

  sudo killall -2 bandwidth > /dev/null 2>&1
  sleep 1
  sudo killall -9 bandwidth > /dev/null 2>&1

  python3 splitftrace.py trace_bandwidth_read.txt
  python3 splitftrace.py trace_bandwidth_write.txt

  mv *.txt res/bandwidth_rw
fi
